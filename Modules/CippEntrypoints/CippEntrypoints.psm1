...
function Receive-CippWellKnownTrigger {
    <#
    .SYNOPSIS
        Serves OAuth 2.0 Protected Resource Metadata (RFC 9728) for the MCP server.
    .DESCRIPTION
        Responds at the root path /.well-known/oauth-protected-resource (and the path-aware
        variant) so MCP clients (e.g. Claude) discover the correct token audience. The advertised
        'resource' is the EasyAuth app's Application ID URI (api://<clientId>) rather than the raw
        server URL, so Entra issues a token whose audience EasyAuth accepts (clears AADSTS9010010).
        Values are derived per-instance from the EasyAuth configuration; nothing is hardcoded.
    .FUNCTIONALITY
        Entrypoint
    #>
    param($Request, $TriggerMetadata)

    # httpContext route params are case-sensitive since PS 7.3; normalize like the main trigger.
    $Request = $Request | ConvertTo-Json -Depth 10 | ConvertFrom-Json
    $Rest = $Request.Params.rest

    if ($Rest -ne 'oauth-protected-resource' -and $Rest -notlike 'oauth-protected-resource/*') {
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{
    ...
    try {
        if ($env:WEBSITE_AUTH_V2_CONFIG_JSON) {
            $AuthConfig = $env:WEBSITE_AUTH_V2_CONFIG_JSON | ConvertFrom-Json
            $AAD = $AuthConfig.identityProviders.azureActiveDirectory
            $SSOClientId = $AAD.registration.clientId
            $Issuer = $AAD.registration.openIdIssuer
        }
    } catch {
        Write-Information "WellKnown: failed to parse EasyAuth config: $($_.Exception.Message)"
    }

    if (-not $Issuer) { $Issuer = "https://login.microsoftonline.com/$($env:TenantID)/v2.0" }

    if (-not $SSOClientId) {
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{
                StatusCode = [HttpStatusCode]::ServiceUnavailable
                Headers    = @{ 'Content-Type' = 'application/json' }
                Body       = (@{ error = 'EasyAuth (SSO) is not configured; cannot publish protected resource metadata.' } | ConvertTo-Json -Compress)
            })
        return
    }

    $Metadata = [ordered]@{
        resource                 = "api://$SSOClientId"
        authorization_servers    = @($Issuer)
        scopes_supported         = @("api://$SSOClientId/user_impersonation")
    ...
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{
            ...
            Body       = ($Metadata | ConvertTo-Json -Depth 5 -Compress)
        })
}

Export-ModuleMember -Function @('Receive-CippHttpTrigger', 'Receive-CippQueueTrigger', 'Receive-CippOrchestrationTrigger', 'Receive-CippActivityTrigger', 'Receive-CIPPTimerTrigger', 'Receive-CippWellKnownTrigger')
