Function Invoke-EditDlpCompliancePolicy {
    <#
    .FUNCTIONALITY
        Entrypoint
    .ROLE
    ...
    try {
        ...
        # If a state was passed, toggle Enabled on the policy
        if ($State) {
            $Params['Enabled'] = ($State -eq 'enable' -or $State -eq $true -or $State -eq 'true')
        }

        # Allow passing arbitrary additional params via Body.parameters
        ...
        $null = New-ExoRequest -tenantid $TenantFilter -cmdlet 'Set-DlpCompliancePolicy' -cmdParams $Params -Compliance -useSystemMailbox $true
        $Result = "Updated DLP compliance policy $Identity"
        Write-LogMessage -Headers $Request.Headers -API $APIName -tenant $TenantFilter -message $Result -Sev Info
        $StatusCode = [HttpStatusCode]::OK
    } catch {
        $ErrorMessage = Get-CippException -Exception $_
        $Result = "Failed updating DLP compliance policy $Identity. Error: $($ErrorMessage.NormalizedError)"
