function Invoke-ExecBaselineRun {
    ...
    try {
        ...
        Start-CIPPBaselineOrchestrator -TenantFilter $TenantFilter -StandardName $StandardName -TemplateId $TemplateId -Mode $Mode -TriggeredBy $User -Force

        $Target = $TemplateId ? "baseline $TemplateId" : ("$($StandardName ? "$StandardName on " : '')$TenantFilter")
        Write-LogMessage -headers $Request.Headers -API $APIName -message "Started baseline $Mode for $Target." -Sev 'Info'
        $Results = [pscustomobject]@{ Results = "Started the $Mode. Results appear on the alignment page as each check completes." }
