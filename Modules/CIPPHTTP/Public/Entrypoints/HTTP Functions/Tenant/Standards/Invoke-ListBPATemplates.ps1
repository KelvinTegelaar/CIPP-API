Function Invoke-ListBPATemplates {
    ...
    .DESCRIPTION
        Lists available Best Practice Analyser (BPA) templates that define which checks to run.
    #>
    [CmdletBinding()]
    param($Request, $TriggerMetadata)
    $Table = Get-CippTable -tablename 'templates'

    $Templates = Get-ChildItem (Join-Path $env:CIPPRootPath 'Config\*.BPATemplate.json') | ForEach-Object {
