function Invoke-ExecAppApprovalTemplate {
    ...
    switch ($Action) {
        'Save' {
            try {
                if ($Request.Body.AppType -eq 'ApplicationManifest' -and $null -ne $Request.Body.ApplicationManifest) {
                    $ManifestProperties = @($Request.Body.ApplicationManifest.PSObject.Properties.Name)
                    $ForbiddenProperties = @('keyCredentials', 'passwordCredentials') | Where-Object { $_ -in $ManifestProperties }
                    if ($ForbiddenProperties.Count -gt 0) {
                        $Body = @{
                            Results = "Application manifest template cannot be saved while these sections exist: $($ForbiddenProperties -join ', '). Remove them and try again."
                        }
