function Invoke-CippTestCopilotReady016 {
    ...
    try {
        ...
        if (-not $AppCounts -or $TotalAppCount -eq 0) {
            $Result = [System.Text.StringBuilder]::new("No Microsoft 365 Copilot usage was detected in the past 30 days.`n`n")
        ...
        $Result = [System.Text.StringBuilder]::new("## Copilot Active Users by App (Last 30 Days)`n`n")
        $null = $Result.Append("| App | Active Users |`n")
        ...
        foreach ($App in ($AppCounts | Sort-Object Value -Descending)) {
            ...
            $null = $Result.Append("| $($AppName.Trim()) | $($App.Value) |`n")
        }
