function Invoke-CippTestGenericTest005 {
    ...
    try {
        ...
        if ($Admins.Count -eq 0) {
            $Result = [System.Text.StringBuilder]::new("No administrator accounts were found in the MFA state data. This is unusual and may indicate the data needs to be re-synced.")
        ...
        if ($NotProtected -gt 0) {
            $null = $Result.Append("**🔴 Critical: $NotProtected admin account(s) have no MFA enforcement.** Admin accounts without MFA are the #1 target for attackers. This should be addressed immediately.`n`n")
        } elseif ($MFARegistered -eq $TotalAdmins) {
            $null = $Result.Append("**✅ All admin accounts have MFA registered and enforced.** Great job keeping your most privileged accounts secured.`n`n")
        }

        $null = $Result.Append("| Display Name | MFA Registered | MFA Method | Protected By | Account Enabled |`n")
        ...
        foreach ($Admin in ($Admins | Sort-Object DisplayName)) {
            ...
            $null = $Result.Append("| $Name | $Registered | $Methods | $Protection | $Enabled |`n")
        }
