function New-GradientServiceSyncRun {
    ...
    try {
        ...
        Write-LogMessage -Headers $Request.Headers -API $APINAME -message "Failed to create tenants in Gradient API. Error: $($_.Exception.Message)" -Sev 'Error' -tenant 'GradientAPI'
    }
    ...
    $LicenseTable = foreach ($singlereq in $RawGraphRequest) {
        ...
        foreach ($sku in $skuid) {
            try {
                ...
                Write-LogMessage -Headers $Request.Headers -API $APINAME -message "Failed to create license in Gradient API. Error: $($_). $results" -Sev 'Error' -tenant $singlereq.tenant
