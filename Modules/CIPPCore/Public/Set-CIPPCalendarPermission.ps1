function Set-CIPPCalendarPermission {
    ...
    try {
        ...
        if ($RemoveAccess) {
            if ($PSCmdlet.ShouldProcess('Remove permissions', "Remove permissions for $RemoveAccess from calendar $UserID")) {
            ...
            if ($PSCmdlet.ShouldProcess('Set permissions', "Set permissions for $UserToGetPermissions on calendar $UserID")) {
                try {
