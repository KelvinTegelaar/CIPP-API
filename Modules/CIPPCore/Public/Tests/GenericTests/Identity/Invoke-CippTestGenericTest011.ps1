function Invoke-CippTestGenericTest011 {
    ...
    try {
        ...
        $StandardsLabelMap = @{}
        if ($ModuleBase) {
            $StandardsJsonPath = Join-Path (Split-Path $ModuleBase -Parent) '..\Config\standards.json'
            if (-not (Test-Path $StandardsJsonPath)) {
                $StandardsJsonPath = Join-Path $ModuleBase '..\..\..\..\Config\standards.json'
            }
