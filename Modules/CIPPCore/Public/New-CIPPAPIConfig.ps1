...
"@
        ...
        if ($resetpassword) {
            ...
            $RemovePasswords = New-GraphGetRequest -uri "https://graph.microsoft.com/v1.0/applications/$($currentapp)/passwordCredentials" -NoAuthCheck $true
            foreach ($Password in $RemovePasswords.value) {
                Write-Host "Removing $($Password.displayName)"
                $null = New-GraphPOSTRequest -type DELETE -uri "https://graph.microsoft.com/v1.0/applications/$($currentapp)/passwordCredentials/$($Password.id)" -NoAuthCheck $true
            }
            $APIPassword = New-GraphPOSTRequest -uri "https://graph.microsoft.com/v1.0/applications/$($currentapp)/addPassword"  -NoAuthCheck $true -type POST -body "{`"passwordCredential`":{`"displayName`":`"Generated by API Setup`",`"startDateTime`":`"2023-07-20T12:47:59.217Z`",`"endDateTime`":`"2033-07-20T12:47:59.217Z`"}}"
