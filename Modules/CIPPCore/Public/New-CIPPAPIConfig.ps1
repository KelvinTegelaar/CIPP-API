...
function New-CIPPAPIConfig {
    ...
    $endDate = (Get-Date).AddYears(10).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ')
...
"@
            ...
            $APIPassword = New-GraphPOSTRequest -uri "https://graph.microsoft.com/v1.0/applications/$($APIApp.id)/addPassword" -NoAuthCheck $true -type POST -body "{`"passwordCredential`":{`"displayName`":`"Generated by API Setup`",`"startDateTime`":`"$startDate`",`"endDateTime`":`"$endDate`"}}"
