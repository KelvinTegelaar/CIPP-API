function Push-Schedulerwebhookcreation {
    ...
    if ($Webhook) {
        ...
        Write-Host "No existing webhook for $($Row.tenant) - $($Row.webhookType) - Time to create."
        try {
            $NewSub = New-CIPPGraphSubscription -TenantFilter $Row.tenant -Resource $Row.webhookType -BaseURL $Row.CIPPURL
            if ($NewSub.Success) {
                Remove-CIPPAzDataTableEntity @Table -Entity $Row
            } else {
                Write-Host "Failed to create webhook for $($Row.tenant) - $($Row.webhookType) - $($_.Exception.Message)"
                Write-LogMessage -message "Failed to create webhook for $($Row.tenant) - $($Row.webhookType)" -Sev 'Error' -LogData $_.Exception
            }
        } catch {
            Write-Host "Failed to create webhook for $($Row.tenant) - $($Row.webhookType): $($_.Exception.Message)"
            Write-LogMessage -message "Failed to create webhook for $($Row.tenant) - $($Row.webhookType)" -Sev 'Error' -LogData $_.Exception
