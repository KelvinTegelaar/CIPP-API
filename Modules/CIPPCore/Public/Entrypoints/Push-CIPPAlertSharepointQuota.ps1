        ...
        if ($sharepointQuota) {
            ...
            if ($QueueItem.value -eq $true){
                if ($UsedStoragePercentage -gt 90) {
                    Write-AlertMessage -tenant $($QueueItem.tenant) -message "SharePoint Storage is at $($UsedStoragePercentage)%. Your alert threshold is 90%"
                }
            }
            elseif ($UsedStoragePercentage -gt $QueueItem.value) {
                Write-AlertMessage -tenant $($QueueItem.tenant) -message "SharePoint Storage is at $($UsedStoragePercentage)%. Your alert threshold is $($QueueItem.value)%"
            }
        }
    } catch {
    }
