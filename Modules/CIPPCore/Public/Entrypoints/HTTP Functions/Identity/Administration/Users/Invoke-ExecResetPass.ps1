...
Function Invoke-ExecResetPass {
    ...
    try {
        $Reset = Set-CIPPResetPassword -userid $Request.query.ID -tenantFilter $TenantFilter -APIName $APINAME -Headers $request.headers.'x-ms-client-principal' -forceChangePasswordNextSignIn $mustChange
