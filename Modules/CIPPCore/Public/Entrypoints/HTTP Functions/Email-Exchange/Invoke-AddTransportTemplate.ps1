...
Function Invoke-AddTransportTemplate {
    ...
    try {
        ...
        Write-LogMessage -user $ExecutingUser -API $APINAME -message "Created Transport Rule Template $($Request.body.name) with GUID $GUID" -Sev Debug
        $body = [pscustomobject]@{'Results' = 'Successfully added template' }

    } catch {
        Write-LogMessage -user $ExecutingUser -API $APINAME -message "Failed to create Transport Rule Template: $($_.Exception.Message)" -Sev Error
        $body = [pscustomobject]@{'Results' = "Template Deployment failed: $($_.Exception.Message)" }
    }
