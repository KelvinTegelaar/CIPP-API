...
Function Invoke-AddTransportRule {
    ...
    $ExetutingUser = $Request.Headers
    Write-LogMessage -Headers$ExetutingUser -API $APINAME -message 'Accessed this API' -Sev 'Debug'
    ...
    $Result = foreach ($tenantFilter in $tenants) {
        ...
        try {
            ...
            Write-LogMessage -Headers$ExetutingUser -API $APINAME -tenant $tenantFilter -message "Created transport rule for $($tenantFilter)" -sev Info
        } catch {
            $ErrorMessage = Get-CippException -Exception $_
            "Could not create transport rule for $($tenantFilter): $($ErrorMessage.NormalizedError)"
            Write-LogMessage -Headers$ExetutingUser -API $APINAME -tenant $tenantFilter -message "Could not create transport rule for $($tenantFilter). Error:$($ErrorMessage.NormalizedError)" -sev Error -LogData $ErrorMessage
        }
    }
