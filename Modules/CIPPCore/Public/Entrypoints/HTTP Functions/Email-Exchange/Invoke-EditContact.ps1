...
Function Invoke-EditContact {
    ...
    try {
        ...
        Write-LogMessage -user $Headers -API $APINAME -tenant $TenantID -message $Results -Sev Info
        $StatusCode = [HttpStatusCode]::OK

    } catch {
        $ErrorMessage = Get-CippException -Exception $_
        $Results = "Failed to edit contact. $($ErrorMessage.NormalizedError)"
        Write-LogMessage -user $Headers -API $APINAME -tenant $TenantID -message $Results -Sev Error -LogData $ErrorMessage
