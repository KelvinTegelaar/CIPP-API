    ...
    try {
        if ($ID) {
            ...
            if ($RunningQueue) {
                ...
                $Queue = New-CippQueueEntry -Name 'Intune Policies - All Tenants' -Link '/endpoint/MEM/list-policies?customerId=AllTenants' -Reference $QueueReference -TotalTasks ($TenantList | Measure-Object).Count
                $Metadata = [PSCustomObject]@{
                    QueueMessage = 'Loading data for all tenants. Please check back in a few minutes'
                    QueueId      = $Queue.RowKey
                }
                $InputObject = [PSCustomObject]@{
                    OrchestratorName = 'IntunePolicyOrchestrator'
                    QueueFunction    = @{
                        ...
                        DurableName  = 'ListIntunePolicyAllTenants'
                    }
                    SkipLog          = $true
                }
