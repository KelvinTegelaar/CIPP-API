...
Function Invoke-ExecNamedLocation {
    ...
    try {
        $results = Set-CIPPNamedLocation -NamedLocationId $NamedLocationId -TenantFilter $TenantFilter -change $change -content $content -Headers $request.headers.'x-ms-client-principal'
    } catch {
