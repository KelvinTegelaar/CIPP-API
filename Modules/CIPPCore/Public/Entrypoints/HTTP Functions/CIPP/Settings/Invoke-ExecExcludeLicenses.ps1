    ...
    try {
        ...
        if ($Request.Query.List) {
            ...
            if ($Rows.Count -lt 1) {
                ...
                Write-LogMessage -API $APIName -headers $Headers -message 'got excluded licenses list' -Sev 'Info'
            }
            $Body = @($Rows)
        }
        ...
        if ($Request.Query.AddExclusion) {
            ...
            Write-LogMessage -API $APIName -headers $Headers -message "Added exclusion $($Request.Body.SKUName)" -Sev 'Info'
        ...
        if ($Request.Query.RemoveExclusion) {
            ...
            Write-LogMessage -API $APIName -headers $Headers -message "Removed exclusion $($Request.Query.GUID)" -Sev 'Info'
            $Body = [pscustomobject]@{'Results' = "Success. We've removed $($Request.Query.GUID) from the excluded list." }
        }
    } catch {
        Write-LogMessage -API $APIName -headers $Headers -message "Exclusion API failed. $($_.Exception.Message)" -Sev 'Error'
