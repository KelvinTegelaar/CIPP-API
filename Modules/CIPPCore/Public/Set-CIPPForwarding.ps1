function Set-CIPPForwarding {
    ...
    try {
        ...
        Write-LogMessage -user $ExecutingUser -API "Forwarding" -message "Set Forwarding for $($username) to $Forward" -Sev "Info" -tenant $TenantFilter
    }
    catch {
        Write-LogMessage -user $ExecutingUser -API "Forwarding" -message "Could not add forwarding for $($username)" -Sev "Error" -tenant $TenantFilter
