function Read-CippMimeMessage {
        ...
        if ($Decoded -match "^([^']*)'[^']*'(.*)$") {
            ...
            try {
                if (![string]::IsNullOrWhiteSpace($Charset)) {
                    # Decode percent-encoded bytes with the declared charset instead of letting
                    # UnescapeDataString assume UTF-16; this avoids double-decoding UTF-8 values.
                    try {
                        $Encoding = [System.Text.Encoding]::GetEncoding($Charset)
                        $PercentBytes = [System.Collections.Generic.List[byte]]::new()
                        $i = 0
                        while ($i -lt $EncodedValue.Length) {
                            if ($EncodedValue[$i] -eq '%' -and ($i + 2) -lt $EncodedValue.Length) {
                                $Hex = $EncodedValue.Substring($i + 1, 2)
                                if ($Hex -match '^[0-9A-Fa-f]{2}$') {
                                    $PercentBytes.Add([Convert]::ToByte($Hex, 16))
                                    $i += 3
                                    continue
                                }
                            }
                            $PercentBytes.Add([byte][char]$EncodedValue[$i])
                            $i++
                        }
                        return $Encoding.GetString($PercentBytes.ToArray())
                    } catch {
    ...
    function Read-CippMimePart {
        ...
        if ($ContentType.Value -eq 'message/rfc822') {
            $Rfc822Bytes = ConvertTo-CippMimeBodyBytes -Body $Entity.Body -TransferEncoding $Entity.Headers['Content-Transfer-Encoding']
            $Rfc822Text = [System.Text.Encoding]::UTF8.GetString($Rfc822Bytes)
            Read-CippMimePart -EntityText $Rfc822Text
            return
        }
