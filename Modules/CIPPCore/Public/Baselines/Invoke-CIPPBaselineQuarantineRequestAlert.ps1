function Invoke-CIPPBaselineQuarantineRequestAlert {
    ...
    .SYNOPSIS
        ...
        alert without discarding recipients it did not add.
    .DESCRIPTION
        ...
        collection, which is precisely the loss the merge exists to prevent.

        Create-vs-update is decided the same way: the alert is looked up by name, and only
        created when it genuinely is not there. Both cmdlets are Security & Compliance only.
    .FUNCTIONALITY
        Internal
    #>
    [CmdletBinding()]
