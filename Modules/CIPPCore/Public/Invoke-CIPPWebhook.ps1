function Invoke-CippWebhook {
    ...
    $Data.ActorIpAddress.Split(".") | ForEach-Object { $IPAsint = 0; $IPAddressByte = 0 } {
    ...
    #0: TenantName + Alert title. E.g. "CyberDrainDev.onmicrosoft.com - New-InboxRule"
    #1: Intro text
    #2: button url
    #3: button text
    #4: after button text
    switch ($data.Operation) {
        ...
        "UserLoggedIn" {
            ...
            $City = $Location.city
            if ($data.UserType -eq 2) {
                $Title = "$($TenantFilter) - an admin account has logged on"
                $IntroText = "$($data.UserId) has logged on from IP $($data.ClientIP) to the application $($Appname). See the table below for more information. $Table"
                $ButtonUrl = "BECURLHERE"
                $ButtonText = "User Management"
            ...
            if ($data.UserType -eq 0) {
                $Title = "$($TenantFilter) - a user has logged on from a potentially unsafe location"
                $IntroText = "$($data.UserId) has logged on from IP $($data.ClientIP) to the application $($Appname). According to our database this is located in $($Country) - $($City). <br/><br> You have set up alerts to be notified when this happens. See the table below for more info.$Table"
                $ButtonUrl = "BECURLHERE"
                $ButtonText = "User Management"
                $AfterButtonText = "<p>If this is incorrect, use the user management screen to block the user and revoke the sessions</p>"
                $HTML = "$HTML" -f $Title, $IntroText, $ButtonUrl, $ButtonText, $AfterButtonText
...
#check if e-mail notification or PSA is configured. If yes, use generated HTML to send e-mail or create PSA ticket

#Check if webhook notification is configured. If yes, send webhook notification with raw data.
