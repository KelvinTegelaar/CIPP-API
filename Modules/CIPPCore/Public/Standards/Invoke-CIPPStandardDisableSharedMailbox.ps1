function Invoke-CIPPStandardDisableSharedMailbox {
    ...
    $UserList = New-GraphGetRequest -uri 'https://graph.microsoft.com/beta/users?$top=999&$filter=accountEnabled eq true and onPremisesSyncEnabled ne true&count=true&ConsistencyLevel=eventual' -Tenantid $tenant -scope 'https://graph.microsoft.com/.default'
