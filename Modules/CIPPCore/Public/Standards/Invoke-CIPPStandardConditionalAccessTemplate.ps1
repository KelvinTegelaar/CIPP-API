    ...
    If ($Settings.remediate -eq $true) {
        ...
        foreach ($Setting in $Settings) {
            try {
                ...
                $null = New-CIPPCAPolicy -TenantFilter $tenant -state $Setting.state -RawJSON $JSONObj -Overwrite $true -APIName $APIName -Headers $request.headers.'x-ms-client-principal' -ReplacePattern 'displayName'
            } catch {
