...
Function Invoke-RemovePolicy {
    ...
    try {
        ...
        Write-LogMessage -user $ExecutingUser -API $APINAME -message $Results -Sev Info -tenant $TenantFilter
        $StatusCode = [HttpStatusCode]::OK

    } catch {
        $ErrorMessage = Get-CippException -Exception $_
        $Results = "Could not delete policy: $($ErrorMessage.NormalizedError)"
        Write-LogMessage -user $ExecutingUser -API $APINAME -message $Results -Sev Error -tenant $TenantFilter -LogData $ErrorMessage
