function Set-CIPPDBCacheUserRegistrationDetails {
    ...
    try {
        ...
        # A language 'foreach' avoids that too, but buffers the whole report first.
        $CachedCount = 0
        $Writer = { Add-CIPPDbItem -TenantFilter $TenantFilter -Type 'UserRegistrationDetails' -AddCount }.GetSteppablePipeline()
        $Writer.Begin($true)
        try {
            New-GraphGetRequest -uri 'https://graph.microsoft.com/beta/reports/authenticationMethods/userRegistrationDetails' -tenantid $TenantFilter -Stream | ForEach-Object {
                $CachedCount++
                $Writer.Process($_)
            }
            # An empty report must not run the end block (it would zero the count row), nor must
            # a part-way failure (its orphan cleanup). Dispose() alone does not run it.
            if ($CachedCount -gt 0) {
                $Writer.End()
                Write-LogMessage -API 'CIPPDBCache' -tenant $TenantFilter -message "Cached $CachedCount user registration details" -sev Debug
            }
        } finally {
            $Writer.Dispose()
        }
