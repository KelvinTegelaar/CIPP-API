...
if ($OnlyAllowInternal -eq "$true") {
    $Results | Add-Member -Type NoteProperty -Name "Results" -Value "Set $($request.query.GroupType) group $($request.query.id) to only allow messages from people inside the organisation."
    ...
    $Results | Add-Member -Type NoteProperty -Name "Results" -Value "Set $($request.query.GroupType) group $($request.query.id) to allow messages from people inside and outside the organisation."
