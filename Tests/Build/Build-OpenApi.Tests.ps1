...
Describe 'incremental build cache' {
    # The dev watcher passes -CachePath so a one-file edit regenerates the spec in a few
    # seconds instead of re-analysing every endpoint. openapi.json is committed and
    # CI-checked, so a cached build MUST be byte-identical to a full one - that is the
    # guarantee these lock in. A nonexistent graph-metadata path keeps them fast; the
    # fixtures have no Graph endpoints, so the graph index never affects their output.
    BeforeAll {
        $script:CacheArgs = @{
            PublicPath        = $script:PublicCopy
            EntrypointPath    = $script:FixtureRoot
            ModulesPath       = $script:ModulesRoot
            OverridePath      = (Join-Path $TestDrive 'empty')
            FrontendPath      = $script:FrontendDir
            GraphMetadataPath = (Join-Path $TestDrive 'no-graph-here')
        }
    }

    It 'a cold cached build is byte-identical to a full build and writes the cache' {
        $Full = Join-Path $TestDrive 'cache-full.json'
        $Cold = Join-Path $TestDrive 'cache-cold.json'
        $Cache = Join-Path $TestDrive 'cold.cache.json'
        & $script:GeneratorPath @script:CacheArgs -OutputPath $Full | Out-Null
        & $script:GeneratorPath @script:CacheArgs -OutputPath $Cold -CachePath $Cache | Out-Null
        (Get-Content $Cold -Raw) | Should -Be (Get-Content $Full -Raw)
        Test-Path $Cache | Should -BeTrue
    }

    It 'a warm build with no changes reuses the cache and stays byte-identical' {
        $Cold = Join-Path $TestDrive 'warm-cold.json'
        $Warm = Join-Path $TestDrive 'warm-warm.json'
        $Cache = Join-Path $TestDrive 'warm.cache.json'
        & $script:GeneratorPath @script:CacheArgs -OutputPath $Cold -CachePath $Cache | Out-Null
        $Out = & $script:GeneratorPath @script:CacheArgs -OutputPath $Warm -CachePath $Cache 6>&1
        (Get-Content $Warm -Raw) | Should -Be (Get-Content $Cold -Raw)
        ($Out -join "`n") | Should -Match 'recomputed 0 endpoint'
    }

    It 'recomputes only the changed endpoint and matches a full rebuild' {
        $Cache = Join-Path $TestDrive 'edit.cache.json'
        $Target = Join-Path $script:FixtureRoot 'Identity/Administration/Invoke-ExecNested.ps1'
        $Original = Get-Content $Target -Raw
        # warm the cache on the unedited tree
        & $script:GeneratorPath @script:CacheArgs -OutputPath (Join-Path $TestDrive 'edit-seed.json') -CachePath $Cache | Out-Null
        try {
            Set-Content -Path $Target -Value ($Original -replace 'Nested access\.', 'Nested access edited.') -NoNewline
            $Inc = Join-Path $TestDrive 'edit-inc.json'
            $Fresh = Join-Path $TestDrive 'edit-fresh.json'
            $Out = & $script:GeneratorPath @script:CacheArgs -OutputPath $Inc -CachePath $Cache 6>&1
            & $script:GeneratorPath @script:CacheArgs -OutputPath $Fresh | Out-Null
            (Get-Content $Inc -Raw) | Should -Be (Get-Content $Fresh -Raw)
            (Get-Content $Inc -Raw) | Should -Match 'Nested access edited'
            ($Out -join "`n") | Should -Match 'recomputed 1 endpoint'
        } finally {
            Set-Content -Path $Target -Value $Original -NoNewline
        }
    }

    It 'falls back to a full build when the cache is from an incompatible version' {
        $Cache = Join-Path $TestDrive 'bad.cache.json'
        Set-Content -Path $Cache -Value '{"version":-1}'
        $Out = Join-Path $TestDrive 'badcache-out.json'
        { & $script:GeneratorPath @script:CacheArgs -OutputPath $Out -CachePath $Cache } | Should -Not -Throw
        (Get-Content $Out -Raw) | Should -Match 'ExecNested'
    }
}
