...
Describe 'Add-CippServerOriginTemplate' {
    It 'replaces generated relative server <Url> with a deployment origin variable' -TestCases @(
        @{ Url = '/api'; Description = 'CIPP API' }
        @{ Url = '/'; Description = 'This CIPP deployment' }
    ) {
        param($Url, $Description)
        $spec = Get-TestSpec -Paths @{ '/api/ListLogs' = @{ get = (Get-Operation) } }
        $spec['servers'] = @([ordered]@{ url = $Url; description = $Description })
...
Describe 'Add-CippResponseSchema - end to end on a temp spec' {
    ...
    It 'reads, enriches, and writes a real file round-trip' {
        ...
        $inputSpec['servers'] = @([ordered]@{ url = '/'; description = 'This CIPP deployment' })
