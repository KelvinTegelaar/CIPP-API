...
try {
    ...
    Log-request "Standards API: $($Tenant) Disabled Password Expiration" -sev Info
}
catch {
    Log-request "Standards API: $($tenant) failed to disable Password Expiration. Error: $($_.exception.message)" -sev Error
}