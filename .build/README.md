...
- `Config/openapi.json` is produced by a private upstream generator that this repository cannot change. A future generator run could restore its old `/api` server URL; the build test will flag the canonical regression, and the public enrichment stage independently normalizes its output back to `{origin}`.
